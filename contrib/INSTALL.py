#!/usr/bin/python
# Author: Anton Mitrokhin, UMD 2016-2017
...
def add_ros_dep(dep):
    DEPS.append("ros-" + ROS_DISTRO + "-" + dep)

def add_dep(dep):
    DEPS.append(dep)

# List  dependencies
add_dep("libeigen3-dev sip-dev libyaml-cpp-dev libboost-dev cmake")
...
def create_startup_scripts():
    ...
    # Startup file with user permissions
    user_format_str = gen_bash_header(HOME_DIR+"/.user_startup.log")
    user_format_str+= ("nohup /root/px4 /root/mainapp.config &>> /dev/null &\n")
    #user_format_str+= ("echo \"$(roslaunch cognifli_bringup minimal.launch &>> $LOGFILE )\"\n")
    write_file_safely(HOME_DIR+"/.user_startup.sh", user_format_str)

    # Startup file with root permissions
